...
bool Node::insertData(uint32_t dataLength, byte* pData, Region& mbr, id_type id, std::stack<id_type>& pathBuffer, byte* overflowTable)
{
		...
		else ////////////changes start from here, when exceed the capacity, and when it is an R tree.
		{
			//NodePtr n;
			//NodePtr nn;
			//split(dataLength, pData, mbr, id, n, nn);//finished here
			...
			//	NodePtr ptrR = m_pTree->m_indexPool.acquire();
			//	if (ptrR.get() == 0)//when is root and is leaf
			//	{
			//		ptrR = NodePtr(new Index(m_pTree, m_pTree->m_rootID, m_level + 1), &(m_pTree->m_indexPool));
			//	}
			//	else//when is root but not leaf, other index nodes exist
			//	{
			//		//ptrR->m_pTree = m_pTree;
			//		ptrR->m_identifier = m_pTree->m_rootID;
			//		ptrR->m_level = m_level + 1;
			...
			//	p->adjustTree(n.get(), nn.get(), pathBuffer, overflowTable);
			//}
			OverflowNode(dataLength, pData, mbr, id);
			return true;
...
void Node::rtreeSplit(uint32_t dataLength, byte* pData, Region& mbr, id_type id, std::vector<uint32_t>& group1, std::vector<uint32_t>& group2)
{
	...
	bzero(mask, m_capacity + 1); //set all of them to be zero

	// insert new data in the node for easier manipulation. Data arrays are always
	// by one larger than node capacity.
	m_pDataLength[m_capacity] = dataLength; //uint32_t*
	m_pData[m_capacity] = pData; //byte** m_pData;// The data stored in the node.
	m_ptrMBR[m_capacity] = m_pTree->m_regionPool.acquire();//RegionPtr* m_ptrMBR;// The corresponding data MBRs.
	...
	*mbr2 = *(m_ptrMBR[seed2]);
	...
	while (cRemaining > 0)
	{
		...
		else
		{
			...
			//update the MBR of the group when new node added
			if (group == 1)
			{
...
void Node::OverflowNode(uint32_t dataLength, byte* pData, Region& mbr, id_type id){
	uint64_t itemNumber = sizeof(m_pTree->table)/sizeof(m_pTree->table[0]);
	bool cur_hasChain=false;
	for(uint64_t cur_identifier=0;cur_identifier<itemNumber;cur_identifier++)
	{
		if(m_pTree->table[cur_identifier].nodeID == m_identifier)
		{
			cur_hasChain = m_pTree->table[cur_identifier].hasChain;
			break;
		}
	}
	...
	if (cur_hasChain==false)
	{
		...
		//assert(m_children >= m_capacity);
		//next = m_pTree->m_leafPool.acquire();
		//if (next.get() == 0) next = NodePtr(new Leaf(m_pTree, -1), &(m_pTree->m_leafPool));
		//next->m_nodeMBR = m_pTree->m_infiniteRegion;
		//next->insertEntry(dataLength, pData, mbr, id);
		Node* newNode; //create new node
		newNode->insertEntry(dataLength, pData, mbr, id);
		m_chainNext = newNode;


		//updatepath
	}
	else //there exists chain
	{
		Node* next = m_chainNext;
		//定位到chain的最后一个node
		while(next->m_chainNext != 0)
		{
			next = next->m_chainNext;
		}
		if (next->m_children >= next->m_capacity)//node is full 
		{
			Node* newChainNode;
			//if (next.get() == 0) next = NodePtr(new Leaf(m_pTree, -1), &(m_pTree->m_leafPool));
			newChainNode->insertEntry(dataLength,pData,mbr,id);
			//id_type cParent = pathBuffer.top(); pathBuffer.pop();
			//NodePtr ptrN = m_pTree->readNode(cParent);
			//ptrN.m_pnext = next;
			next->m_chainNext = newChainNode;
		}
		else
			next->insertEntry(dataLength,pData,mbr,id);
		delete next;
...
bool Node::balanceInsert(uint32_t dataLength, byte* pData, Region& mbr, id_type id, std::stack<id_type>& pathBuffer, byte* overflowTable)
{
	if (m_children < m_capacity)
	{
		bool adjusted = false;
