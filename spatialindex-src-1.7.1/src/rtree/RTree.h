...
namespace SpatialIndex
{
	namespace RTree
	{
		...
		public:
			...
			/////////////
			virtual void balanceInsert(uint32_t len, const byte* pData, const IShape& shape, id_type shapeIdentifier);
			...
			//Tools::PoolPointer<Node> getmleafpool()
			//{
			//	return m_leafPool;
			//}
...
void refreshTable();
		...
		private:
			...
			///////////////////
			void balanceInsert_impl(uint32_t dataLength, byte* pData, Region& mbr, id_type id);
			void balanceInsert_impl(uint32_t dataLength, byte* pData, Region& mbr, id_type id, uint32_t level, byte* overflowTable);
