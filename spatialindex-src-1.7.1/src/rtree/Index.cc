...
uint32_t Index::findLeastEnlargement(const Region& r) const
{
	...
	RegionPtr t = m_pTree->m_regionPool.acquire();//新建的，用于后面的region out
...
//得到每个孩子的最新mbr
id_type cur_identifier = m_pIdentifier[cChild];

				/////////////////////////////////////////////////////////}

			//check if it has a chain
			//if has, reconstruct the MBR
			...
			//uint32_t newDimension = table[cur_identifier].m_dimension;
			//double* newHigh = table[cur_identifier].m_pHigh;
			//double* newLow = table[cur_identifier].m_pLow;
			Region newMBR = m_pTree->table[cur_identifier].m_mbr;
			newMBR.getCombinedRegion(*t, r);
			double a = newMBR.getArea();
...
else if (enl == area)
		{
			...
			if (a < newMBR.getArea()) best = cChild;
		}
