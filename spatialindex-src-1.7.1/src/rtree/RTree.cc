...
void SpatialIndex::RTree::RTree::balanceInsert(uint32_t len, const byte* pData, const IShape& shape, id_type id)
{
	if (shape.getDimension() != m_dimension) throw Tools::IllegalArgumentException("insertData: Shape has the wrong number of dimensions.");
	...
	try
	{
		...
		balanceInsert_impl(len, buffer, *mbr, id);
		// the buffer is stored in the tree. Do not delete here.
...
void SpatialIndex::RTree::RTree::insertData_impl(uint32_t dataLength, byte* pData, Region& mbr, id_type id)
{
	...
	try
	{
		...
		if (l.get() == root.get())// the get method returns a type Node*
		{
			assert(root.unique());
			root.relinquish();
		}
...
void SpatialIndex::RTree::RTree::balanceInsert_impl(uint32_t dataLength, byte* pData, Region& mbr, id_type id)
{
	assert(mbr.getDimension() == m_dimension);
	...
	try
	{
		...
		l->balanceInsert(dataLength, pData, mbr, id, pathBuffer, overflowTable);
...
void SpatialIndex::RTree::RTree::balanceInsert_impl(uint32_t dataLength, byte* pData, Region& mbr, id_type id, uint32_t level, byte* overflowTable)
{
	assert(mbr.getDimension() == m_dimension);
	...
	n->balanceInsert(dataLength, pData, mbr, id, pathBuffer, overflowTable);
}
...
void SpatialIndex::RTree::RTree::rangeQuery(RangeQueryType type, const IShape& query, IVisitor& v)
{
	...
	try
	{
		...
		uint64_t itemNumber = sizeof(root->m_pTree->table)/sizeof(root->m_pTree->table[0]);
		bool cur_hasChain=false;
		uint64_t cur_identifier;
		for(cur_identifier=0;cur_identifier<itemNumber;cur_identifier++)
		{
			if(root->m_pTree->table[cur_identifier].nodeID == root->m_identifier)
			{
				cur_hasChain = root->m_pTree->table[cur_identifier].hasChain;
				break;
			}
		}
		Region rootMbr;
		if(cur_hasChain == true)//check the table
		{
			rootMbr= table[cur_identifier].m_mbr;
			if (root->m_children > 0 && query.intersectsShape(rootMbr)) st.push(root);
		}
		else
		{
		...
		while (! st.empty())
		{
			...
			//当是leaf node. 叶子结点的entry的mbr都是新的.
			if (n->m_level == 0)
			{
				v.visitNode(*n);
				//检查当前叶子结点有没有chain
				...
				for(cur_identifier=0;cur_identifier<itemNumber;cur_identifier++)
				{
					if(root->m_pTree->table[cur_identifier].nodeID == n->getmidentifier())
					{
						cur_hasChain = root->m_pTree->table[cur_identifier].hasChain;
						break;
					}
				...
				else //当cur_hasChain == true, 有chain
				{
					//要访问叶子结点的entry，也要访问chain node的entry
					//访问叶子结点的entry
					for (uint32_t cChild = 0; cChild < n->m_children; ++cChild)
					{
						bool b;
					...
					//访问chain node的entry
					Node* c = n->m_chainNext;
					while(c != 0)
					{
						for (uint32_t cChild = 0; cChild < c->m_children; ++cChild)
						{
							bool b;
							if (type == ContainmentQuery) b = query.containsShape(*(c->m_ptrMBR[cChild]));
							else b = query.intersectsShape(*(c->m_ptrMBR[cChild]));

							if (b)
							{
								Data data = Data(c->m_pDataLength[cChild], c->m_pData[cChild], *(c->m_ptrMBR[cChild]), c->m_pIdentifier[cChild]);
								v.visitData(data);
								++(m_stats.m_u64QueryResults);
							}
					...
					//			else
					//			{
					//				uint64_t childid = n->m_pIdentifier[cChild];
					//				/*uint64_t cur_identifier;
					//				for(cur_identifier=0;cur_identifier<itemNumber;cur_identifier++)
					//				{
					//					if(root->m_pTree->table[cur_identifier].nodeID == childid)
					//					{
					//						cur_hasChain = root->m_pTree->table[cur_identifier].hasChain;
					//						break;
					//					}
					...
					//				if(cur_hasChain == true)//check the table
					//				{
					//					childMbr= table[cur_identifier].m_mbr;
					//				}

					//				b = query.intersectsShape(childMbr);
					//				if (b)
					//				{
					//					Data data = Data(n->m_pDataLength[cChild], n->m_pData[cChild], childMbr, n->m_pIdentifier[cChild]);
					//					//Data data = Data(n->m_pDataLength[cChild], n->m_pData[cChild], *(n->m_ptrMBR[cChild]), n->m_pIdentifier[cChild]);
					//					v.visitData(data);
					//					++(m_stats.m_u64QueryResults);
					//				}
					...
					//				///////////////////读chain中的内容
					//				//LeafPtr l = static_cast<Leaf*>n;
					//				
					//Leaf* L = static_cast<Leaf*>(n.get());
					//Node * c_node = L->chain;
					//id_type c_id;
					//uint64_t itemNumber = sizeof((L->getmptree())->table)/sizeof((L->getmptree())->table[0]);
					//Region chainMbr;
					//while(c_node->m_chainNext != 0)//这里的next==0
					//{
					//	c_id = c_node->getmidentifier();
					//	//chain node的MBR本来就是新的
					//	chainMbr = c_node->getmnodembr();





					//	b = query.intersectsShape(chainMbr);
					//	if (b)
					//	{
					//		Data data = Data(n->m_pDataLength[c_id], n->m_pData[cChild], childMbr, n->m_pIdentifier[cChild]);
					//		//Data data = Data(n->m_pDataLength[cChild], n->m_pData[cChild], *(n->m_ptrMBR[cChild]), n->m_pIdentifier[cChild]);
					//		v.visitData(data);
					//		++(m_stats.m_u64QueryResults);
					//	}
			...
			else
			{
				...
				for (uint32_t cChild = 0; cChild < n->m_children; ++cChild)
				{
					...
					for(uint64_t cur_identifier=0;cur_identifier<itemNumber;cur_identifier++)
					{
						if(root->m_pTree->table[cur_identifier].nodeID == cChild)
						{
							cur_hasChain = root->m_pTree->table[cur_identifier].hasChain;
							break;
						}
					...
					if(cur_hasChain == true)//check the table
					{
						Region childMbr= table[cur_identifier].m_mbr;
						if (query.intersectsShape(childMbr))
		...
		void SpatialIndex::RTree::RTree::GlobalRebalance()
		{
			//读出树的所有leaf node们
			Tools::PointerPool<SpatialIndex::RTree::Node> leaves = m_leafPool;
			Leaf* leaf = static_cast<Leaf*>((leaves.get()).get());//得到leaves中的第一个leaf，leaves的get会pop()
			//插入每一个叶子结点的chain node
			while(leaf!=0)
			{
				Node* c_chain = leaf->chain;
			while(c_chain!=0)
			{
				for (uint32_t cChild = 0; cChild < c_chain->m_children; ++cChild)
				{
					
					balanceInsert(c_chain->m_pDataLength[cChild], c_chain->m_pData[cChild], c_chain->m_ptrMBR[cChild], c_chain->m_pIdentifier[cChild]);
				
				}
				c_chain = c_chain->m_chainNext;
			}
			leaf = static_cast<Leaf*>((leaves.get()).get());
			}
			//更新table
			refreshTable();
		}
		...
		void SpatialIndex::RTree::RTree::refreshTable()
		{
