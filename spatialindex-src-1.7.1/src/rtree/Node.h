...
namespace SpatialIndex
{
	namespace RTree
	{
		...
		public:
			...
			id_type getmchildren()
			{
				return m_children;
			}

				// Specifies the node capacity.
			uint32_t getmcapacity()
			{
				return m_capacity;
			}
			...
			RegionPtr* getmptrmbr()
			{
				return m_ptrMBR;
			}
				// The corresponding data identifiers.
			id_type* getmpidentifier()
			{
				return m_pIdentifier;
			}

			uint32_t* getmpdatalength()
			{
				return m_pDataLength;
			}

			uint32_t getmtotaldatalength()
			{
				return m_totalDataLength;
			}

			void OverflowNode(uint32_t dataLength, byte* pData, Region& mbr, id_type id);
		...
		private:
			...
			//原始插入算法，这儿没有加virtual
			bool balanceInsert(uint32_t dataLength, byte* pData, Region& mbr, id_type id, std::stack<id_type>& pathBuffer, byte* overflowTable);
			...
			uint32_t m_children;
				// The number of children pointed by this node.
			//对于chain node，这个是the number of entry in this chain node
